...
Digraph DotMergerPass::GeneratePattern() {
  ...
  auto* matmul_0 =
      builder.AddInstr("matmul", std::vector<PatternVar const*>{in_0, in_1}, std::vector<PatternVar const*>{out_0})
          ->set_label("matmul_0");
  auto* matmul_1 =
      builder.AddInstr("matmul", std::vector<PatternVar const*>{in_0, in_2}, std::vector<PatternVar const*>{out_1})
          ->set_label("matmul_1");
  return builder.release();
};

void DotMergerPass::Match(Program* prog) {
    ...
    for (size_t i = 0; i < prog->size(); ++i) {
      ...
      if (nodes_to_remove.find(instr.get()) != nodes_to_remove.end()) {
        if (++cnt == nodes_to_remove.size()) {
          auto concat_out = builder.Concat({in_1, in_2}, 1);
          auto matmul_out = builder.Matmul(in_0, concat_out);
